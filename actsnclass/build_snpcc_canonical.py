...
def plot_snpcc_train_canonical(sample: Canonical, output_plot_file=''):
    ...
    plt.figure()
    plt.subplot(1, 3, 1)
    plt.plot(ztrain_axis, np.exp(log_dens_ztrain), label='train')
    plt.plot(zcanonical_axis, np.exp(log_dens_zcanonical), label='canonical')
    plt.xlabel('redshift')
    plt.legend()

    plt.subplot(1, 3, 2)
    plt.plot(gtrain_axis, np.exp(log_dens_gtrain), label='old train')
    plt.plot(gcanonical_axis, np.exp(log_dens_gcanonical), label='new train')
    plt.xlabel('g_peakmag')
    plt.xlim(15, 35)
    plt.legend()

    plt.subplot(1, 3, 3)
    plt.plot(rtrain_axis, np.exp(log_dens_rtrain), label='old train')
    plt.plot(rcanonical_axis, np.exp(log_dens_rcanonical), label='new train')
    plt.xlabel('r_peakmag')
    plt.xlim(15, 35)
    plt.legend()
